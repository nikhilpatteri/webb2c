angular.module('replayroutedetail', [])
	.controller('ReplayRouteDetailCtrl', function ($scope, $rootScope, $timeout, PageConfig, UtilsFactory, BatsServices,
		...
		$scope.getHistory = function (timeSlot) {
			...
			BatsServices.history(inputParam).success(function (response) {
				...
				if ($scope.historyVal.values != "") {
					...
					for (i in svg) {
						icons[i] = {
							path: svg[i].path, fillColor: svg[i].fillColor, scale: .7, strokeColor: 'white', strokeWeight: .10, fillOpacity: 1, offset: '5%',
							anchor: new google.maps.Point(10, 25) // orig 10,50 back of
						};
					}
		 ...
		 *  event at a place 
		 */
		function getEventsMarker(EventData) {
			var unicEvent = _.uniq(EventData);
			console.log(angular.toJson(unicEvent));
			var finalEventsBasedOnLatLong = [];
			var eventString = "";
			var infowindow = new google.maps.InfoWindow();
			for (var i = 0; i < unicEvent.length; i++) {
				var tempLat = unicEvent[i].lat;
				var tempLong = unicEvent[i].long;

				// for (var j = 0; j < unicEvent.length; j++) {
				// 	console.log(j + " = " + eventString);
				// 	if (tempLat === unicEvent[j].lat && tempLong === unicEvent[i].long) {
				// 		eventString = eventString + "\n " + BatsServices.matchAlarm(unicEvent[j].alarm_type);
				// 		console.log(eventString + unicEvent[j].alarm_type);
				// 	}

				// 	// if (j === unicEvent.length - 1) {

				// 	// 	//finalEventsBasedOnLatLong.push

				// 	// }
				// 	// if (j === unicEvent.length - 1) {
				// 	// 	console.log(eventString + "inside if for cleaning");
				// 	// 	eventString = "";
				// 	// }
				// }
				console.log(eventString + ' inside mappimg marker ');
				var markerLat = Number(unicEvent[i].lat);
				var markerLng = Number(unicEvent[i].long);
				...
				google.maps.event.addListener(marker, 'click', (function (marker, i) {
					return function () {
						...
						for (var j = 0; j < unicEvent.length; j++) {
							//console.log(j + "  " + eventString);
							if (tempLat === unicEvent[j].lat && tempLong === unicEvent[j].long) {
								console.log(i + " " + j);
								console.log(unicEvent[j].alarm_type + unicEvent[i].alarm_type)
								eventString = eventString + "\n <br> " + BatsServices.matchAlarm(unicEvent[j].alarm_type);
								console.log(eventString + unicEvent[j].alarm_type);
							}
							$scope.eventValue = eventString;
							console.log("event name while adding to marker " + $scope.eventValue + eventString);//checkEventFilter(values[i].alarm_type)
				...
				//}
				// 		$scope.eventValue = eventString;
				// 		//	console.log("event name while adding to marker " + $scope.eventValue + eventString + unicEvent[j].alarm_type + unicEvent[i].alarm_type);
		...
		function getEventString(values, markerLat, markerLng, event) {
			...
			for (var j = 0; j < values.length; j++) {
				...
				if (markerLat === Number(values[j].lat) && markerLng === Number(values[j].long)) {
					...
					eventString = eventString + "\n <br> " + BatsServices.matchAlarm(values[j].alarm_type);
		...
		function setEventMarkers(value) {
			//getEventsMarker(values);
			//var values = _.uniq(value,'lat','long','alarm_type');
			// for(var i=0;i<value.length;i++){
			// 	if( _.isEqual(value[i],value[i+1])){
			...
			console.log("it result ))) " + angular.toJson(values));
			//console.log(angular.toJson(values) + "\n" + angular.toJson(value));
			var infowindow = new google.maps.InfoWindow();
